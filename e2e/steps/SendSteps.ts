...
import { HomeFlows, clickByText, idShouldExist, insertTextById, textShouldExist } from "../helpers"

Given("The user is in the send screen", { timeout: -1 }, async function () {
    await HomeFlows.goToSend()
})

Given("The user selects {string} token to be sent", { timeout: -1 }, async function (token: string) {
    await clickByText(token)
})

When("The user inserts a very small amount to be sent", { timeout: -1 }, async function () {
    await insertTextById("0.00001", "SendScreen_amountInput")

    await clickByText("Send")
})

When("The user inserts the amount {string} to be sent", { timeout: -1 }, async function (amount: string) {
...
When("The user inserts the address {string} of the receiver", { timeout: -1 }, async function (address: string) {
    await insertTextById(address, "InsertAddressSendScreen_addressInput")
})

Then("The user can click next button to go to the next screen", { timeout: -1 }, async function () {
...
When("The user inserts a comma on the amount to be sent", { timeout: -1 }, async function () {
    await insertTextById("0,1", "SendScreen_amountInput")

    await clickByText("Send")
})

Then("The user sees the amount with the comma converted to a dot", { timeout: -1 }, async function () {
...
Then("The user should see NFT sent with success message", { timeout: -1 }, async function () {
    await idShouldExist("informUserForOutgoingNFTSuccessToast", {
        timeout: 20000,
    })
})

Then("The user should see NFT incoming with success message", { timeout: -1 }, async function () {
