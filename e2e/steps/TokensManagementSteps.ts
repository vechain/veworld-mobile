...
When("The user selects {string} token from the unselected tokens list", { timeout: -1 }, async (token: string) => {
    await clickByText(token)
})

Then("The user should see {string} token balance in home screen", { timeout: -1 }, async (token: string) => {
    await goBack()
...
Then("The user should not see {string} token balance in home screen", { timeout: -1 }, async (token: string) => {
    await goBack()
    await textShouldNotExist(token)
})

When("The user type the query {string}", { timeout: -1 }, async (query: string) => {
    await insertTextById(query, "ManageTokenScreen_SearchInput_searchTokenInput")

    await clickByText("Manage Tokens")
})

Then("The list should display {string} and not {string}", { timeout: -1 }, async (token: string, token2: string) => {
    await textShouldExist(token)
    await textShouldNotExist(token2)
})

When("The user add a custom token with address {string}", { timeout: -1 }, async (address: string) => {
    await clickById("ManageTokenScreen_AddCustomToken_Button")
    await insertTextById(address, "AddCustomTokenBottomSheet-TextInput-Address")
    await clickByText("Add")
})

Then("The user should see {string} custom token balance in home screen", { timeout: -1 }, async (token: string) => {
    await textShouldExist(token)
})
...
When(
    ...
    async (address1: string, address2: string) => {
        await clickById("ManageTokenScreen_AddCustomToken_Button")
        await insertTextById(address1, "AddCustomTokenBottomSheet-TextInput-Address")
        await clickByText("Add")
        await clickById("ManageTokenScreen_AddCustomToken_Button")
        await insertTextById(address2, "AddCustomTokenBottomSheet-TextInput-Address")
        await clickByText("Add")
        await goBack()
    },
)
...
When("The user delete custom token {string}", { timeout: -1 }, async (token1: string) => {
