...
Given("The user is in the contacts management screen", { timeout: -1 }, async () => {
...
When("The user deletes the contact with name {string}", { timeout: -1 }, async (name: string) => {
    await ContactsManagementFlows.deleteContact(name)
})

When("the user adds {string} contacts", { timeout: -1 }, async (count: string) => {
    for (let i = 0; i < Number(count) && i < EXAMPLE_CONTACTS.length; i++) {
        await ContactsManagementFlows.goToAddContactScreen()
        await ContactsManagementFlows.addNewContact(EXAMPLE_CONTACTS[i].name, EXAMPLE_CONTACTS[i].address)
    }
})

Then("The user should see contact with name {string} in contacts list", { timeout: -1 }, async (name: string) => {
    await textShouldBeVisible(name, { timeout: LONG_TIMEOUT })
})

Then("The user should not see contact with name {string} in contacts list", async (name: string) => {
...
Then("the user should be able to scroll to the contact {string}", { timeout: -1 }, async (name: string) => {
