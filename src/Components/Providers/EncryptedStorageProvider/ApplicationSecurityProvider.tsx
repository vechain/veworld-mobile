...
import { BiometricsUtils, debug, error, HexUtils, warn, info } from "~Utils"
    ...
    useEffect(() => {
        if (triggerAutoLock) {
            try {
                ...
                error("Failed to lock application", e)
            } finally {
                setTriggerAutoLock(false)
            }
        }
    }, [triggerAutoLock, currentState, walletStatus, lockApplication])
