...
export const TransactionStatusBox: React.FC<Props> = memo(({ status }) => {
    ...
    const statusTitle =
        status === ActivityStatus.PENDING ? LL.ACTIVITIES_STATUS_pending() : LL.ACTIVITIES_STATUS_failed()
            ...
            <BaseView mx={16} my={12}>
                <BaseView flexDirection="row">
                    <BaseView bg={statusTitleBgColor} borderRadius={6}>
                        <BaseText py={3} px={6} typographyFont="captionMedium" color={COLORS.WHITE}>
                            {statusTitle}
                        </BaseText>
                    </BaseView>
                </BaseView>
