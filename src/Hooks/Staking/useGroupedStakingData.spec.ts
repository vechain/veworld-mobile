...
describe("useGroupedStakingData", () => {
    ...
    it("should group nodes by owner address", () => {
        const { result } = renderHook(() => useGroupedStakingData(testUserAddress), {
            wrapper: TestWrapper,
            initialProps: {
                preloadedState,
        ...
        expect(result.current.stakingGroups).toHaveLength(3)
        expect(result.current.totalGroups).toBe(3)

        // Find groups by address
        const ownedGroup = result.current.stakingGroups.find(
            group => group.address === "0xCF130b42Ae33C5531277B4B7c0F1D994B8732957",
        )
        const managedGroup1 = result.current.stakingGroups.find(group => group.address === "0x456")
        const managedGroup2 = result.current.stakingGroups.find(group => group.address === "0x789")

        expect(ownedGroup).toBeDefined()
        expect(managedGroup1).toBeDefined()
        expect(managedGroup2).toBeDefined()
        ...
        expect(managedGroup1!.nodes).toHaveLength(1) // node 2
        expect(managedGroup1!.isOwner).toBe(false)
        expect(managedGroup1!.nodes[0].nodeId).toBe("2")

        expect(managedGroup2!.nodes).toHaveLength(1) // node 4
        expect(managedGroup2!.isOwner).toBe(false)
        expect(managedGroup2!.nodes[0].nodeId).toBe("4")
    })
    ...
    it("should correctly filter NFTs for each group", () => {
        ...
        // Managed group should have NFT for node 2
        expect(managedGroup1!.nfts).toHaveLength(1)
        expect(managedGroup1!.nfts[0].tokenId).toBe("2")
    })
    ...
    it("should prioritize owned nodes first in the order", () => {
        ...
        expect(result.current.stakingGroups[0].address).toBe("0xCF130b42Ae33C5531277B4B7c0F1D994B8732957")

        // Subsequent groups should be managed nodes (isOwner: false)
        expect(result.current.stakingGroups[1].isOwner).toBe(false)
        expect(result.current.stakingGroups[2].isOwner).toBe(false)
    })
