...
export const getThreeRandomIndexes = (): number[] => {
    ...
    // it should be 'maxIndex - 2' but let's keep it under 6, so it generates more separated numbers
    const maxIndexForFirstNumber = 4
    const firstNumber = Math.floor(CryptoUtils.random() * maxIndexForFirstNumber + 1)
    result.push(firstNumber)

    // generate second number
    const minIndexForSecondNumber = firstNumber + 1
    // it should be 'maxIndex - 1' but let's keep it under 9, so it generates more separated numbers
    const maxIndexForSecondNumber = 8
    const getSecondNumber = () =>
        Math.floor(CryptoUtils.random() * (maxIndexForSecondNumber - minIndexForSecondNumber + 1)) +
    ...
    const getThirdNumber = () =>
        Math.floor(CryptoUtils.random() * (maxIndexForThirdNumber - minIndexForThirdNumber + 1)) +
