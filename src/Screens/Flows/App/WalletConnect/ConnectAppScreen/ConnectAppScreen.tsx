...
export const ConnectAppScreen: FC<Props> = ({ route }: Props) => {
    ...
    const processProposal = useCallback(async () => {
        ...
        for (const requiredNamespace in requiredNamespaces) {
            const _namespace = requiredNamespaces[requiredNamespace]

            const requiredChains =
                _namespace.chains ??
                networks.map(net => `vechain:${net.genesis.id.slice(-32)}`)

            namespaces[requiredNamespace] = {
                accounts: requiredChains.map(
                    chain => `${chain}:${selectedAccount.address}`,
                ),
                methods: _namespace.methods,
                events: _namespace.events,
            }
        }
