...
export const Favourites = React.memo(
    ({ title, actionLabel, bookmarkedDApps, onActionLabelPress, onDAppPress }: FavouritesProps) => {
        const showBookmarkedDAppsList = bookmarkedDApps.length > 0

        return (
            <BaseView py={24}>
                <BaseView flexDirection="row" justifyContent="space-between" px={24}>
                    <BaseText typographyFont="bodyBold">{title}</BaseText>

                    <BaseTouchable action={onActionLabelPress}>
                        <BaseText typographyFont="body">{actionLabel}</BaseText>
                    </BaseTouchable>
                </BaseView>
