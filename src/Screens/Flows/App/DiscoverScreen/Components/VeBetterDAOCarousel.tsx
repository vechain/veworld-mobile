...
export const VeBetterDAOCarousel = () => {
    ...
    const defaultSortedSlides = useMemo(() => {
        return [...activeSlides].sort(applyDefaultSorting)
    }, [activeSlides])

    const sortedSlides = useMemo(() => {
        return defaultSortedSlides.sort((a, b) => {
            return (
                (bannerInteractions[b.name ?? ""]?.amountOfInteractions ?? 0) -
                (bannerInteractions[a.name ?? ""]?.amountOfInteractions ?? 0)
            )
        })
    }, [bannerInteractions, defaultSortedSlides])
