...
export const BASE_GAS_PRICE_FN_SIG = "0x000000000000000000000000000000000000626173652d6761732d7072696365"
...
export const useCalculateGas = ({ token }: { token: FungibleTokenWithBalance }) => {
    ...
    const calculateFee = useCallback(
        async (gas: BigNumber | number, gasPriceCoef: number): Promise<BigNumber> => {
    ...
    useEffect(() => {
        if (token.symbol === VTHO.symbol) {
            calculateFee(VTHO_GAS_ESTIMATE, DEFAULT_GAS_COEFFICIENT).then(fee => {
                setVthoEstimate(fee)
            })
        }
    }, [calculateFee, token])
