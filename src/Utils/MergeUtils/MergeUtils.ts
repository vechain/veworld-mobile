...
export const mergeObjects = <T>(obj1: T, obj2: T, ignoredFields: (keyof T)[] = []): T => {
...
export const mergeArrays = <T>(
    ...
    const fromArr1 = arr1.map(item1 => {
        const matchingObjFromArr2 = arr2.find(item2 => item2[uniqueKey] === item1[uniqueKey])
        if (!matchingObjFromArr2) return item1

        return _.mergeWith({}, item1, matchingObjFromArr2, (objValue, srcValue, key) => {
            if (ignoredFields.includes(key as keyof T)) {
    ...
    const fromArr2 = arr2.filter(item2 => !arr1.some(item1 => item1[uniqueKey] === item2[uniqueKey]))

    let result = [...fromArr1, ...fromArr2]

    // If orderBy is specified, then sort
    if (orderBy) {
        const orderKeys: (keyof T)[] = orderBy.map(item => (Array.isArray(item) ? item[0] : item))
        const orderDirections: ("asc" | "desc")[] = orderBy.map(item => (Array.isArray(item) ? item[1] : "asc"))
...
export const mergeNftCollections = (collections1: NftCollection[], collections2: NftCollection[]): NftCollection[] => {
