...
describe("getContractAddresses", () => {
    it("should return the contract addresses for the owner address", async () => {
        ...
        const contractAddresses = await getContractAddresses(NETWORK_TYPE.MAIN, ownerAddress, 10, 0)

        expect(contractAddresses).toEqual(responseData)
        expect(axios.get).toHaveBeenCalledWith(NFT_CONTRACTS_FOR_ADDRESS(NETWORK_TYPE.MAIN, ownerAddress, 10, 0), {
